...
export default class Database {
    ...
    async fetchRobots(): Promise<TwinsRobot[]> {
        const picking = await getDocs(collection(this.db, 'iota', 'robot', 'picking'));
        const packing = await getDocs(collection(this.db, 'iota', 'robot', 'packing'));
        const delivery = await getDocs(collection(this.db, 'iota', 'robot', 'delivery'));
    ...
    async updateRobotStatus(id: number, type: RobotType, isActive: boolean) {
        ...
        await updateDoc(doc(this.db, 'iota', 'robot', type.toLowerCase(), id.toString()), {
            id,
            isActive,
        });
    }
    ...
    async fetchOrders(wallet: string): Promise<Order[]> {
        const snapshot = await getDocs(collection(this.db, 'iota', 'users', wallet));
    ...
    async addOrUpdateOrder(wallet: string, order: Order) {
        await this.authenticate();
        const ref = doc(this.db, 'iota', 'users', wallet, order.id);
